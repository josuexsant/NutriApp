import connection from "../models/DataBase.js"; // Ajusta la ruta si es necesario
import bcrypt from "bcrypt";
import * as auth from "../utils/auth.js"; // Si es necesario

const nutriologoRegister = async (req, res) => {
	const { name, lastname1, lastname2, phoneNumber, state, city, license, email, password, confirmPassword } =
		req.body;
	...
	try {
		...
		await connection.query("CALL verificar_unicidad_nutriologo(:correo, :cedula, :telefono)", {
		...
		const newNutriologo = await connection.query(
