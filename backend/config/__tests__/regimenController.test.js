...
const agregarRegimenConsumoDiario = async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.execute(
      ...
      [
        pacienteId, 
        JSON.stringify(diasJson),  // Se asegura de que los JSON se envíen como cadenas JSON
        JSON.stringify(caloriasPorGrupoJson), 
        JSON.stringify(alimentosJson)
      ]
    );
  ...
  try {
    ...
    const [rows] = await connection.execute(
      ...
      [
        pacienteId, 
        limiteCalorias, 
        JSON.stringify(tiemposComidaJson),  // Se asegura de que los JSON se envíen como cadenas JSON
        JSON.stringify(caloriasPorGrupoJson), 
        JSON.stringify(alimentosTiemposJson)
      ]
    );
